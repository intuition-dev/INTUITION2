...
let webAdmin = new WebAdmin(apiProtocol, apiHost, apiPort);
...
class Posts {
    ...
    showSubDirs(id) {
        ...
        let listTemp = '';
        return webAdmin.getSubDirsList(id)
            .then(dirs => {
                dirs.forEach(el => {
    ...
    showMd(id, pathPrefix) {
        ...
        webAdmin.getPostMd(id, pathPrefix)
            .then(post => {
    ...
    uploadFile(input, pathPrefix) {
        ...
        return webAdmin.upload(data, pathPrefix);
    }

    setPublishDate(publishDate, pathPrefix) {
        return webAdmin.setPublishDate(publishDate, pathPrefix);
    }

    MbakeVersion() {
        return webAdmin.getMbakeVersion();
    }
