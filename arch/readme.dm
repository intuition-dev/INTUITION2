...
it has a (/admin/editors/....) datatable (not in a tag) as main ui. it cruds users.
you can change admin password: edit admin yaml.
it will have a client_admin_api.js
...
it will have a client_editors_api.js
a user of CMS will have to download the auth_key.json from FB and place in root.
...
ports & view page
---------------
each web app will host ports.json, it does not need to be edited if you don't change ports in admin:
with these fields:
admin_api_port: XX
editor_api_port: XX
...
editors directory
-----------------
There may be need to have more data on editor in webapp
(/editors/dir/vic, editors/dir/wolfgang)
It is just a page/screen.
and v2



This makes it easy to extend for any use.
Base.ts will have FB_auth helper functions.
